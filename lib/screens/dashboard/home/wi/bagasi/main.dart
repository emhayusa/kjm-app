...
import 'detail_cek_bagasi.dart';
import 'form_cek_bagasi.dart';
import 'dart:convert';

import 'package:shared_preferences/shared_preferences.dart';

class ListBagasi extends StatefulWidget {
  const ListBagasi({super.key});

  @override
  State<ListBagasi> createState() => _ListBagasiState();
}

class _ListBagasiState extends State<ListBagasi> {
  late DateTime selectedDate;
  bool isLoading = false;
  TextEditingController searchController = TextEditingController();
  List<CekBagasiModel> datas = [];
  ...
  List<CekBagasiModel> filteredDatas = [];
  ...
  Future<void> fetchData() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<CekBagasiModel> tamuList =
            data.map((json) => CekBagasiModel.fromJson(json)).toList();
  ...
  void navigateToFormKendaraan() async {
    await Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => FormCekBagasi()),
    );
                  ...
                  : filteredDatas.length == 0
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (BuildContext context, int index) {
                            CekBagasiModel data = filteredDatas[index];
                                ...
                                child: ListTile(
                                  ...
                                  onTap: () {
                                    Navigator.push(
                                      context,
                                      MaterialPageRoute(
                                          builder: (context) => DetailCekBagasi(
                                                data: data,
                                              )),
                                    );
                                  },
